# Open Cookie Consent — Specification (v0.1)
...
- **Categories**:  
  ...
  - Analytics (toggle).  
  - Advertising (toggle).  
  - *(Extensible via filter for future categories.)*  
...
- Asset budget: **JS ≤ 12 KB gzipped**, **CSS ≤ 5 KB** for MVP.  
...
## 14) Roadmap
- **v0.1 (this spec)**: UI, Consent Engine, GCMv2, Plausible, scheduled static scan, OCD snapshot, conditional banner, updated‑cookies notice.  
- **v0.2**: runtime sampler, admin diff UI, email/webhook notify, Cloudflare purge.  
- **v0.3**: export/import, WP‑CLI, multisite defaults, icon‑drawer API.  
- **v1.0**: polish, docs site, translations infra, security review.

---

## 15) Acceptance Criteria (MVP v0.1)
- Banner appears **only when non‑essential detected** (unless admin forces always‑on).  
- “Accept All”, “Reject Non‑Essential”, “Preferences/Save” work via mouse & keyboard, with SR support.  
- Non‑essential scripts never run before consent; run after grant; GCM defaults set before tags.  
- Updated‑cookies notice shows on version mismatch with buttons **Accept All / Review Cookies / Keep Current Settings**; new items remain **denied** unless granted.  
- Plausible events fire when enabled; no server writes by default.  
- All strings are translatable; admin uses WP Settings API; inputs sanitised.  
- Asset budgets met; no per‑click origin hits; compatible with Cloudflare Enterprise.
